      ...
      for (int i(d->Ng); i < d->Nx-d->Ng; i++) {
        for (int j(d->Ng); j < d->Ny-d->Ng; j++) {
          for (int k(d->Ng); k < d->Nz-d->Ng; k++) {
            ...
            d->aux[ID(ISCE::TDerivs::dtD+var, i, j, k)] = -fnet[ID(var, i, j, k)];
          }
        }
      }
    }
    ...
    for (int var(0); var < d->Ncons; var++) {
      for (int i(d->Ng); i < d->Nx-d->Ng; i++) {
        for (int j(d->Ng); j < d->Ny-d->Ng; j++) {
          ...
          d->aux[ID(ISCE::TDerivs::dtD+var, i, j, 0)] = -fnet[ID(var, i, j, 0)];
        }
      }
    }
    free(fx);
  ...
  else
  {
    ...
    for (int var(0); var < d->Ncons; var++) {
      for (int i(d->Ng); i < d->Nx-d->Ng; i++) {
        ...
        d->aux[ID(ISCE::TDerivs::dtD+var, i, 0, 0)] = -fnet[ID(var, i, 0, 0)];
      }
    }
    free(fx);
  }
