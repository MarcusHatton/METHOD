    ...
    for (int j(0); j < d->Ny; j++) {
      for (int k(0); k < d->Nz; k++) {
        ...
        if (d->y[j] > 0.01*sin(2*PI*d->x[i])) {
          d->prims[ID(3, i, j, k)] = 0.3;
          d->prims[ID(0, i, j, k)] = vShear;
          d->prims[ID(5, i, j, k)] = rho0 + rho1;
  ...
  for (int i(0); i < d->Nx; i++) {
    for (int j(0); j < d->Ny; j++) {
      for (int k(0); k < d->Nz; k++) {
        ...
        d->prims[ID(3, i, j, k)] = 0.3;
        d->prims[ID(0, i, j, k)] = vShear * tanh(d->y[j]/a);
        d->prims[ID(5, i, j, k)] = rho0 + rho1 * tanh(d->y[j]/a);
        d->prims[ID(1, i, j, k)] = 0.0;
...
Hot_Static_Star::Hot_Static_Star(Data * data) : InitialFunc(data)
{
  ...
  if (d->xmin != -3.0 || d->xmax != 3.0) throw std::invalid_argument("Domain has incorrect values. Expected x E [0.0, 1.0]\n");
  if (d->ymin != -3.0 || d->ymax != 3.0) throw std::invalid_argument("Domain has incorrect values. Expected y E [0.0, 1.0]\n"); 
  if (d->zmin != 3.0 || d->zmax != 3.0) throw std::invalid_argument("Domain has incorrect values. Expected z E [0.0, 1.0]\n"); 
