...
Smeared_Shocktube_1D_Para::Smeared_Shocktube_1D_Para(Data * data) : InitialFunc(data)
{
  ...
  for (int i(0); i<d->Nx; i++) {
    for (int j(0); j<d->Ny; j++) {
      for (int k(0); k<d->Nz; k++) {
        d->prims[ID(3, i, j, k)] = 5.5 + 4.5*tanh(-10*(d->x[i] - 0.5));
        d->prims[ID(5, i, j, k)] = 5.5 + 4.5*tanh(-10*(d->x[i] - 0.5));
...
BDNK_Smeared_Shocktube::BDNK_Smeared_Shocktube(Data * data) : InitialFunc(data)
{
  // Syntax
  Data * d(data);
  //if (d->gamma != 5.0/3.0) throw std::invalid_argument("Expected the index gamma = 5/3\n");
