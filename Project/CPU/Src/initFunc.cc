  ...
  for (int i(0); i < d->Nx; i++) {
    for (int j(0); j < d->Ny; j++) {
      for (int k(0); k < d->Nz; k++) {
        ...
        d->prims[ID(0, i, j, k)] = Tmin + (Tmax - Tmin) * (tanh((d->x[i]-x_l)/transition_width) + tanh((x_r-d->x[i])/transition_width)) / 2;
        d->prims[ID(0, i, j, k)] += -data->optionalSimArgs[1]*initial_flux*(6*PI*cos(6*PI*d->x[i])); // Off-set of temp. from boundary layer analysis
      }
    }
  }
}
...
//  if (d->Nprims > 15) throw std::invalid_argument("Trying to implement a single fluid initial state on incorrect model.\n\tModel has wrong number of primitive variables to be single fluid model.");
  ...
  if (d->ymin != -2.0 || d->ymax != 2.0) throw std::invalid_argument("Domain has incorrect values. Expected y E [-2.0, 2.0]\n");
  ...
  for (int i(0); i < d->Nx; i++) {
    for (int j(0); j < d->Ny; j++) {
      for (int k(0); k < d->Nz; k++) {
        ...
        if (fabs(d->y[j]) > 1.0 + 0.01*sin(2*PI*d->x[i])) {
...
ShearTest_StepFunction::ShearTest_StepFunction(Data * data) : InitialFunc(data)
{
  // Syntax
  Data * d(data);
  //if (d->gamma != 5.0/3.0) throw std::invalid_argument("Expected the index gamma = 5/3\n");
  ...
  // Limit checking
  if (d->xmin != -1.0 || d->xmax != 1.0) throw std::invalid_argument("Domain has incorrect values. Expected x E [0.0, 1.0]\n");
  //if ((d->ymin != 0.0 || d->ymax != 1.0) && dir==1) throw std::invalid_argument("Domain has incorrect values. Expected y E [0.0, 1.0]\n");
  //if ((d->zmin != 0.0 || d->zmax != 1.0) && dir==2) throw std::invalid_argument("Domain has incorrect values. Expected z E [0.0, 1.0]\n");
  ...
  for (int i(0); i<d->Nx; i++) {
    for (int j(0); j<d->Ny; j++) {
      for (int k(0); k<d->Nz; k++) {
        d->prims[ID(3, i, j, k)] = 1.0; // p
        d->prims[ID(5, i, j, k)] = 1.0; // n
        d->prims[ID(0, i, j, k)] = 0;
        d->prims[ID(2, i, j, k)] = 0;
        if (d->x[i] < 0.0) {
          d->prims[ID(1, i, j, k)] = -0.1; //vy
        } 
        else {
          d->prims[ID(1, i, j, k)] = 0.1;  //vy           
        } 
