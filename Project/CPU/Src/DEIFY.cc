...
*/
  ...
  for (int i(d->is); i < d->ie; i++) {
	...
	/*
	// By hand with big simplifications (from paper)
        aux[ID(TDerivs::dtp, i, j, k)] = (1-d->gamma)*(1+aux[ID(TDerivs::dtE, i, j, k)]);
        aux[ID(TDerivs::dtrho, i, j, k)] = aux[ID(TDerivs::dtE, i, j, k)];
        aux[ID(TDerivs::dtv1, i, j, k)] = (1/(prims[ID(Prims::rho, i, j, k)]+prims[ID(Prims::p, i, j, k)]))*(-prims[ID(Prims::v1, i, j, k)]*((2-d->gamma)*aux[ID(TDerivs::dtE, i, j, k)] + (1-d->gamma)*aux[ID(TDerivs::dtD, i, j, k)])
					  + aux[ID(TDerivs::dtS1, i, j, k)]);
        aux[ID(TDerivs::dtv2, i, j, k)] = (1/(prims[ID(Prims::rho, i, j, k)]+prims[ID(Prims::p, i, j, k)]))*(-prims[ID(Prims::v2, i, j, k)]*((2-d->gamma)*aux[ID(TDerivs::dtE, i, j, k)] + (1-d->gamma)*aux[ID(TDerivs::dtD, i, j, k)])
					  + aux[ID(TDerivs::dtS2, i, j, k)]);
        aux[ID(TDerivs::dtv3, i, j, k)] = (1/(prims[ID(Prims::rho, i, j, k)]+prims[ID(Prims::p, i, j, k)]))*(-prims[ID(Prims::v3, i, j, k)]*((2-d->gamma)*aux[ID(TDerivs::dtE, i, j, k)] + (1-d->gamma)*aux[ID(TDerivs::dtD, i, j, k)])
					  + aux[ID(TDerivs::dtS3, i, j, k)]);
	*/
