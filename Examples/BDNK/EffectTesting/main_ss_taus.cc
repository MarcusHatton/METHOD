...
int main(int argc, char *argv[]) {
  ...
  float taus[] = {10.0,1.0,1e-1,1e-2,1e-3};
  float tau = 0;

  for(int i=0; i<5; i++) {
    tau = taus[i];
    cout << tau << std::endl;
    std::string dirpath = "./1d/ideal/stillshock/taus/"+std::to_string(tau);
  ...
  const std::vector<double> toy_params           { {1.0e-4, tau,  1.0e-2, tau,  1e-3, tau} };
  ...
  SerialSaveDataHDF5 save(&data, &env, "1d/ideal/stillshock/taus/"+std::to_string(tau)+"/data_serial_TIx_0", SerialSaveDataHDF5::OUTPUT_ALL);
  ...
  for (int n(0); n<nreports; n++) {
    ...
    SerialSaveDataHDF5 save_in_loop(&data, &env, "1d/ideal/stillshock/taus/"+std::to_string(tau)+"/data_serial_TIx_"+std::to_string(n+1), SerialSaveDataHDF5::OUTPUT_ALL);
    sim.evolve(output);
