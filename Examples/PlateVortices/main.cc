...
int main(int argc, char *argv[]) {
  ...
  IS model(&data); // bool for alt_C2P - gone
  ...
  SerialSaveDataHDF5 save(&data, &env, "output/data_serial_0", SerialSaveDataHDF5::OUTPUT_ALL);
  ...
  for (int n(0); n<nreports; n++) {
    ...
    SerialSaveDataHDF5 save_in_loop(&data, &env, "test/data_serial_"+std::to_string(n+1), SerialSaveDataHDF5::OUTPUT_ALL);
    sim.evolve(output);
