...
int main(int argc, char *argv[]) {
  ...
  bool output(false);
  int nreports(5);
  std::string data_dir = "/scratch/mjh1n20/KHI/MIS/Ideal/t30/";
  ...
  const std::vector<double> toy_params           { {1e-15, 5.0e-1,  1e-15, 5.0e-1,  1.0e-15, 5.0e-1} };
...
//  IS_Shocktube_1D init(&data);
  ...
  ParallelSaveDataHDF5 save(&data, &env, data_dir+"dp_"+std::to_string(nx)+"x"+std::to_string(ny)+"x"+std::to_string(nz)+"_0", ParallelSaveDataHDF5::OUTPUT_ALL);
  ...
  for (int n(0); n<nreports; n++) {
    ...
    ParallelSaveDataHDF5 save_in_loop(&data, &env, data_dir+"dp_"+std::to_string(nx)+"x"+std::to_string(ny)+"x"+std::to_string(nz)+"_"+std::to_string(n+1), ParallelSaveDataHDF5::OUTPUT_ALL);
    sim.evolve(output);
