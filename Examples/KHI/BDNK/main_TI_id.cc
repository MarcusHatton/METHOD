...
int main(int argc, char *argv[]) {
  ...
  const std::vector<double> toy_params           { {1.0e-15, 5.0e-1,  1.0e-15, 5.0e-1,  1.0e-15, 1.0e-2} };
...
//  IS_Shocktube_1D init(&data);
  ...
  ParallelSaveDataHDF5 save(&data, &env, "2d/Ideal/TI/dp_"+std::to_string(nx)+"x"+std::to_string(ny)+"x"+std::to_string(nz)+"_0", ParallelSaveDataHDF5::OUTPUT_ALL);
  ...
  for (int n(0); n<nreports; n++) {
    ...
    ParallelSaveDataHDF5 save_in_loop(&data, &env, "2d/Ideal/TI/dp_"+std::to_string(nx)+"x"+std::to_string(ny)+"x"+std::to_string(nz)+"_"+std::to_string(n+1), ParallelSaveDataHDF5::OUTPUT_ALL);
    sim.evolve(output);
