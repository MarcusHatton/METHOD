...
int main(int argc, char *argv[]) {
  ...
  const std::vector<double> toy_params           { {1.0e-15, 1.0e-1,  1.0e-2, 5.0e-3,  1.0e-15, 1.0e-1} };
  ...
  Shocktube init(&data, 0); //direction given by second arg (int)
  ...
  SerialSaveDataHDF5 save(&data, &env, "1d/Bulk/1em2_5em3/ds_"+std::to_string(nx)+"_0", SerialSaveDataHDF5::OUTPUT_ALL);
  ...
  for (int n(0); n<nreports; n++) {
    ...
    SerialSaveDataHDF5 save_in_loop(&data, &env, "1d/Bulk/1em2_5em3/ds_"+std::to_string(nx)+"_"+std::to_string(n+1), SerialSaveDataHDF5::OUTPUT_ALL);
    sim.evolve(output);
