...
int main(int argc, char *argv[]) {
  ...
  const std::vector<double> toy_params           {1.0e-15, 1.0e-1,  5.0e-2, 1.0e1,  1.0e-15, 1.0e-1};
  ...
  SerialSaveDataHDF5 save(&data, &env, "1d/Bulk/5em2_1e1/ds_"+std::to_string(nx)+"_0", SerialSaveDataHDF5::OUTPUT_ALL);
  ...
  for (int n(0); n<nreports; n++) {
    ...
    SerialSaveDataHDF5 save_in_loop(&data, &env, "1d/Bulk/5em2_1e1/ds_"+std::to_string(nx)+"_"+std::to_string(n+1), SerialSaveDataHDF5::OUTPUT_ALL);
    sim.evolve(output);
