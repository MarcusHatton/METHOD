...
int main(int argc, char *argv[]) {
  ...
  const std::vector<double> toy_params           { {kappa, 1.0e-1,  1e-15, 1.0e-1,  1e-15, 1.0e-1} };
  ...
  for (int n(0); n<nreports; n++) {
    ...
    SerialSaveDataHDF5 save_in_loop(&data, &env, "1d/kappa/stillshock/kappas/"+std::to_string(kappa)+"/data_serial_TIx_"+std::to_string(n+1), SerialSaveDataHDF5::OUTPUT_ALL);
    sim.evolve(output);
