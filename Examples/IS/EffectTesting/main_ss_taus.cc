...
int main(int argc, char *argv[]) {
  ...
  for(int i=0; i<5; i++) {
    tau = taus[i];
    cout << tau << std::endl;
    std::string dirpath = "./1d/StillShock/All/tau_scaling/"+std::to_string(tau);
  ...
  const std::vector<double> toy_params           { {5.0e-3, tau,  5.0e-2, tau,  1e-15, tau} };
  ...
  SerialSaveDataHDF5 save(&data, &env, "1d/StillShock/All/tau_scaling/"+std::to_string(tau)+"/ds_0", SerialSaveDataHDF5::OUTPUT_ALL);
  ...
  for (int n(0); n<nreports; n++) {
    ...
    SerialSaveDataHDF5 save_in_loop(&data, &env, "1d/StillShock/All/tau_scaling/"+std::to_string(tau)+"/ds_"+std::to_string(n+1), SerialSaveDataHDF5::OUTPUT_ALL);
    sim.evolve(output);
